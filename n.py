...
def nems():
	print W+B+"[*]"+W+" NABIL Writting All ID_Name From Your Friendlist ..."
...
def name():
	print W+B+"[*]"+W+" NABIL Writting All Name From Your Friendlist ..."
...
def emailfromid():
	id=input(W+R+'[?]'+W+' ID>> ')
	print W+B+"[*]"+W+" Nabil TryingGetting Email From ID "+R+str(id)+" ..."
...
def getemail():
	...
	print W+B+"[*]"+W+" Nabil Geeting Email ..."
	print W+R+"="*40
	data= requests.get('https://graph.facebook.com/me/friends?access_token='+str(open('my_token.txt').readline().replace('\n','')+''))
	dd=json.loads(data.text)
	for k in dd['data']:
...
def getusername():
	...
	print W+B+"[*]"+W+" Nabil Geetting USERNAME ...\n"
	data= requests.get('https://graph.facebook.com/me/friends?access_token='+str(open('my_token.txt').readline().replace('\n','')+''))
	dd=json.loads(data.text)
	for k in dd['data']:
		r2=requests.get('https://graph.facebook.com/'+k['id']+'?access_token='+str(open('my_token.txt').readline().replace('\n',''))+"")
...
def getnum():
	...
	try:
		...
		print W+B+"[+] "+W+js["name"]+" ~> "+R+"FucK Him Not Using Number."
		
def get_number():
	print W+B+"[*]"+W+" Wait Nabil Provide U All Numbers ..."
...
def main():
	...
	print W+R+ "NOTE:"+C+"NABIL SAY U CAN USE REAL ACC"
	print W+R+ "NOTE:"+C+"ONLY FOR EDUCATON PERPORCE " 
	print W+R+"[-] "+C+"Select From Menu:\n"
	print W+B+"  A"+R+")"+G+" NABIL SAY LOGING WITH UR FB ID"
	...
	if pilih == "a" or pilih == "A":
		...
		try:
			...
			print W+R+"[-]"+W+" NABIL SAY WAIT."
			raw_input(W+R+'[!]'+W+' Press enter to menu ...')
			os.system('clear')
			main()
		print W+B+"[+] "+W+"Nabil Was Grabbing ID ..."
		try:
	...
	Buy 50000k This Tool Contact Me FACEBOOK Nabil Rahman
		"""
		raw_input( W+"[!]"+R+" Press enter to menu ...")
		os.system('clear')
		main()
		...
		print """
	DEV\t  : NABIL RAHMAN (TEAM-404)
	Coder\t  : Nabil Rahman
	Pronhub\t  : I DonT Watch Pron Vedio
	WebSite  : www.xnxx.com
		
	(C) Copyright EDIT 5050 By NABIL RAHMAN
	 	 """
	 	raw_input( W+R+"[!]"+W+" Press enter to menu ...")
	 	os.system('clear')
	 	main()
